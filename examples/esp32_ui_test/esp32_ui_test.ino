    ...
    "layout" : [
      {
        ...
        "layout" : [
          ...
          },
          {
            "type" : "number",
            "properties" : {
              "label": "Number (step 0.1, readonly)",
              "id" : "number_ro",
              "step" : ".1",
              "value" : "-99.9",
              "update" : "false",
...
constexpr auto SLIDER = "slider";
constexpr auto NUMBER_SLIDER = "number_slider";
constexpr auto BUTTON_CLICKED = "button_clicked";
constexpr auto BUTTON_EDGES = "button_edges";
...
void eventCallback(JsonDocument& event_doc) {
  ...
  if (strncmp(event, EVENT_INPUT, strlen(EVENT_INPUT)) == 0) {
    ...
    if (strncmp(id, NUMBER, strlen(NUMBER)) == 0) {
      const char* value = event_doc[PAYLOAD][VALUE];
      const char SET_VALUE[] = R"RAW(#{ "command": "set_value", "payload": { "id" : "number_ro", "value" : "%s" } })RAW";
      char set_value[sizeof(SET_VALUE) + strlen("+999.9")];
      snprintf(set_value, sizeof(set_value), SET_VALUE, value);
      Serial.println(set_value);
      pTxCharacteristic->setValue(String(set_value));
      pTxCharacteristic->notify();
      delay(50);  // avoid congestions // TODO handshake "X"
    }
    if (strncmp(id, SLIDER, strlen(SLIDER)) == 0) {
      const char* value = event_doc[PAYLOAD][VALUE];
      const char SET_VALUE[] = R"RAW(#{ "command": "set_value", "payload": { "id" : "number_slider", "value" : "%s" } })RAW";
      char set_value[sizeof(SET_VALUE) + strlen("+99")];
      snprintf(set_value, sizeof(set_value), SET_VALUE, value);
      Serial.println(set_value);
      pTxCharacteristic->setValue(String(set_value));
      pTxCharacteristic->notify();
    ...
    if (strncmp(id, BUTTON_CLICKED, strlen(BUTTON_CLICKED)) == 0) {
      const char* value = event_doc[PAYLOAD][VALUE];
      const char SET_VALUE[] = R"RAW(#{ "command": "set_value", "payload": { "id" : "text_events", "value" : "%s" } })RAW";
      char set_value[sizeof(SET_VALUE) + strlen(CLICKED)];
      snprintf(set_value, sizeof(set_value), SET_VALUE, value);
      Serial.println(set_value);
      pTxCharacteristic->setValue(String(set_value));
      pTxCharacteristic->notify();
    ...
    if (strncmp(id, BUTTON_EDGES, strlen(BUTTON_EDGES)) == 0) {
      const char* value = event_doc[PAYLOAD][VALUE];
      const char SET_VALUE[] = R"RAW(#{ "command": "set_value", "payload": { "id" : "text_events", "value" : "%s" } })RAW";
      char set_value[sizeof(SET_VALUE) + strlen(RELEASED)];
      snprintf(set_value, sizeof(set_value), SET_VALUE, value);
      Serial.println(set_value);
      pTxCharacteristic->setValue(String(set_value));
      pTxCharacteristic->notify();
    ...
    if (strncmp(id, TEXT, strlen(TEXT)) == 0) {
      const char* value = event_doc[PAYLOAD][VALUE];
      const char SET_VALUE[] = R"RAW(#{ "command": "set_value", "payload": { "id" : "text_events", "value" : "%s" } })RAW";
      char set_value[sizeof(SET_VALUE) + TEXT_MAX_LENGTH];
      snprintf(set_value, sizeof(set_value), SET_VALUE, value);
      Serial.println(set_value);
      pTxCharacteristic->setValue(String(set_value));
      pTxCharacteristic->notify();
...
void loop() {
  ...
  if (connected && update) {
      Serial.println("ping");
      const char PING[] = R"RAW(#{ "command": "ping" })RAW";
      pTxCharacteristic->setValue(String(PING));
      pTxCharacteristic->notify();
      delay(50);  // avoid congestions // TODO handshake "X"
